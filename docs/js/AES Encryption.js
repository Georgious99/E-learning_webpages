...
function encryptText() {
    ...
    {
      encrypted = CryptoJS.AES.encrypt(text, password, { mode: CryptoJS.mode.ECB }).toString();
    }
  ...
  function decryptText() {
    ...
    if (mode === "CBC") 
    {
      decrypted = CryptoJS.AES.decrypt(encrypted, password, { mode: CryptoJS.mode.CBC }).toString(CryptoJS.enc.Utf8);
    } 
    else if (mode === "ECB")
    {
      decrypted = CryptoJS.AES.decrypt(encrypted, password, { mode: CryptoJS.mode.ECB }).toString(CryptoJS.enc.Utf8);
    }
    if(encrypted==="")
    {
